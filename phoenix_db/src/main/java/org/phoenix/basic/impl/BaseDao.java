...
public class BaseDao<T> implements IBaseDao<T> {
	...
	protected Session getSession() {
		try{
			...
			if(hb.getMessage().equals("No Session found for current thread")){
				Session session = PerThreadSessionUtil.getSession();
				synchronized (this){
					if(session == null){
						session = sessionFactory.openSession();
						System.err.println("creat session in thread local");
						PerThreadSessionUtil.addSession(session);
					}
				}
				return session
						;
